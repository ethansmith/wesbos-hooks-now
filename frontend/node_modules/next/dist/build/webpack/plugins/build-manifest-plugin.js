"use strict";exports.__esModule=true;exports.default=void 0;var _webpackSources=require("webpack-sources");var _constants=require("next-server/constants");// This function takes the asset map generated in BuildManifestPlugin and creates a
// reduced version to send to the client.
const generateClientManifest=(assetMap,isModern)=>{const clientManifest={};const appDependencies=new Set(assetMap.pages['/_app']);delete assetMap.pages['/_app'];Object.entries(assetMap.pages).forEach(([page,dependencies])=>{// Filter out dependencies in the _app entry, because those will have already
...
if(filteredDeps.length){clientManifest[page]=filteredDeps;}});return JSON.stringify(clientManifest);};// This plugin creates a build-manifest.json for all assets that are being output
...
.reduce((a,c)=>(a[c]=assetMap.pages[c],a),{});compilation.assets[_constants.BUILD_MANIFEST]=new _webpackSources.RawSource(JSON.stringify(assetMap,null,2));if(this.clientManifest){const clientManifestPath=`${_constants.CLIENT_STATIC_FILES_PATH}/${this.buildId}/_buildManifest.js`;compilation.assets[clientManifestPath]=new _webpackSources.RawSource(`self.__BUILD_MANIFEST = JSON.parse('${generateClientManifest(assetMap,false)}')
            self.__BUILD_MANIFEST_CB && self.__BUILD_MANIFEST_CB()`);if(this.modern){const modernClientManifestPath=`${_constants.CLIENT_STATIC_FILES_PATH}/${this.buildId}/_buildManifest.module.js`;compilation.assets[modernClientManifestPath]=new _webpackSources.RawSource(`self.__BUILD_MANIFEST = JSON.parse('${generateClientManifest(assetMap,true)}')
              self.__BUILD_MANIFEST_CB && self.__BUILD_MANIFEST_CB()`);}}callback();});}}exports.default=BuildManifestPlugin;