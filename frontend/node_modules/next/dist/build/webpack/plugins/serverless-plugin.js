...
return source;}const b1=source.slice(0,index);const b2=source.slice(index+target.length);const nextBuffer=bufferTee(b2);return Buffer.concat([b1,replacement,nextBuffer],index+replacement.length+nextBuffer.length);}return bufferTee(buffer);}function interceptFileWrites(compiler,contentFn){compiler.outputFileSystem=new Proxy(compiler.outputFileSystem,{get(target,propKey){const orig=target[propKey];if(propKey!=='writeFile'){return orig;}return function(targetPath,content,...args){return orig.call(target,targetPath,contentFn(content),...args);};}});}class ServerlessPlugin{constructor(buildId,{isServer,isTrace,isFlyingShuttle}){this.buildId=void 0;this.isServer=void 0;this.isTrace=void 0;this.isFlyingShuttle=void 0;this.buildId=buildId;this.isServer=isServer;this.isTrace=isTrace;this.isFlyingShuttle=isFlyingShuttle;}apply(compiler){if(!this.isServer){if(this.isFlyingShuttle){compiler.hooks.emit.tap('ServerlessPlugin',compilation=>{const assetNames=Object.keys(compilation.assets).filter(f=>f.includes(this.buildId));for(const name of assetNames){compilation.assets[name.replace(new RegExp(`${this.buildId}[\\/\\\\]`),'client/').replace(/[.]js$/,`.${this.buildId}.js`)]=compilation.assets[name];}});}return;}interceptFileWrites(compiler,content=>replaceInBuffer(content,NEXT_REPLACE_BUILD_ID,this.buildId));if(!this.isTrace){compiler.hooks.compilation.tap('ServerlessPlugin',compilation=>{compilation.hooks.optimizeChunksBasic.tap('ServerlessPlugin',chunks=>{chunks.forEach(chunk=>{// If chunk is not an entry point skip them
